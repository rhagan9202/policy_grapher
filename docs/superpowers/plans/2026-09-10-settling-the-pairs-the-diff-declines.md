...
- Produces: `migrate_pairing_decisions(driver, database: str) -> dict[str, int]` returning exactly `{"converted", "retired_same_edition", "retired_conflicting", "implements_deleted", "proposals_deleted"}`; the `:RetiredLinkDecision` label (all original properties intact plus `retired_reason` ∈ `{'converted', 'same_edition', 'conflicting_pairing'}`); the startup call in `main.py` after `apply_schema`, counts logged.
