...
| STORY-050 | The codebase contains no code the application cannot reach | — | S | 3 | An AST sweep on 2026-08-21 found public symbols with no caller in `src/`. **Narrowed at the STORY-048 design session:** this story covers only the genuinely dead ones — `attach_authority`, `merge_authority` and `merge_entity` in `versions.py`, which [architecture.md](../specs/architecture.md) already admits nothing populates, and `text_of` in `sources/pdf.py`, superseded by `pages_of`. The extraction cache (`CachedExtractor`, `GraphCacheStore`, `CacheStore`) is **explicitly excluded**: STORY-048 wires it in sprint 4, and deleting it now to satisfy this story's title would be gaming the story |
