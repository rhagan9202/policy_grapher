...
| ID | Item | Notes |
| --- | --- | --- |
| STORY-114 | The pairing queue reaches a class of decline larger than one page | The `outcome` filter lands with the pairing split and makes each class reachable in one request, but the screen passes no `limit`, so within a class the cap is still 50. The consequence a reviewer meets: a `partner_taken` row's own instruction — "mark that pairing distinct first" — is unexecutable from the screen when the winning `auto_paired` row sits past row 50 of its class. Paging or a raised request; the render cost of 500 rows was measured at roughly 13,500 DOM nodes, so the answer is probably paging |
| STORY-115 | A verdict costs a diff, and the settled list is unbounded | Measured during the pairing split, and they compound: the queue GET runs `diff_versions` inside a write transaction at a cost quadratic in unsettled candidates (37ms at 100 pending, 549ms at 1600), while `SETTLED` has no `LIMIT` and each row draws 27 DOM nodes, a textarea and two buttons. One click on a large edition pair disables every button, re-runs the diff and re-renders the whole list. The fix is a server-side `limit` on `SETTLED` mirroring `CANDIDATES`, plus an `ORDER BY` when that lands |
| STORY-116 | Above-threshold candidate recording is unbounded | Everything at or above `PAIRING_CONFIDENCE` is recorded unconditionally — deliberate, since §2 of the pairing design makes reaching an `auto_paired` row to *undo* it the point — but it is O(unmatched × unmatched): 900 persisted edges measured for one 30-clause wholly-renumbered edition, 14,400 at 120×120, rewritten on every Triage GET. Bounding it is a design round, not a patch |
| STORY-117 | Deleting a document leaves its obligations behind | Pre-existing and now reached from three directions: a `PAIRING_CANDIDATE` edge between two orphaned obligations is unreachable by any drop; the pairing migration counts such decisions as `decisions_missing_documents` and deliberately cannot classify them; and the same orphan class already strands `:Change` nodes. The repair is `DELETE_DOCUMENT` cascading to `:MANDATES`-ed obligations |
| STORY-118 | A section holding two reviewer-rejected pairings still reads as the machine's ambiguity | `SETTLED_DISTINCT` says a reviewer declined a pairing when a section holds exactly one unmatched clause each side. With two `distinct` verdicts in one section, pass 2 declines on arity without consulting either verdict, so all four rows keep `AMBIGUOUS_SECTION` — literally true, and it blames the pairing rule for a decision a person made. Deliberately not fixed during the split: the obvious replacement would assert a reviewer settled *the* two clauses, which is false when only two of four possible pairings were judged |
