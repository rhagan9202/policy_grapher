...
| STORY-048 | An ingested edition's derived layer can be built from the running app | — | L | 3 | **The surge's spine.** `rebuild_derived`, `embed_chunks`, `CachedExtractor` and `GraphCacheStore` have no caller anywhere in `src/` — only in tests. Nothing in the running application can turn an ingested edition into obligations, embeddings or proposals, so Triage, Review and Ask can only ever be empty and the vector leg of hybrid retrieval is permanently inert. The 2026-08-21 audit could only exercise those screens by running Python by hand against the container's Neo4j. See [STORY-048](stories/STORY-048-derived-layer-buildable-from-the-app.md) |
| STORY-049 | A cold start produces a corpus with text in it | — | M | 3 | `maybe_autoingest` loads `SAMPLE_CSV`, and the manifest path deliberately records no edition and no text ([ADR-011](../specs/adr/ADR-011-instruments-have-versions.md)). So `docker compose up` from a clean volume yields 439 documents, zero chunks and zero obligations — measured 2026-08-21. Every screen DI-2 added is blank on a first run, which is also what anyone evaluating this repo sees first. Needs auto-ingest to seed at least one PDF edition, or a documented one-command path that does. Depends on STORY-048 for the derived half |
| STORY-050 | The codebase contains no code the application cannot reach | — | S | 3 | An AST sweep on 2026-08-21 found several public symbols with no caller in `src/`: the extraction cache (`CachedExtractor`, `GraphCacheStore`, `CacheStore`) that [ADR-013](../specs/adr/ADR-013-extraction-is-a-port-with-a-ratchet.md) devotes a third of itself to; `attach_authority`, `merge_authority` and `merge_entity` in `versions.py`, which [architecture.md](../specs/architecture.md) already admits nothing populates; and `text_of` in `sources/pdf.py`, superseded by `pages_of`. Each is either wired up or deleted — tested-but-unreachable is the worst of both, since the tests report health for something no user can run. Partly resolved by STORY-048 |
