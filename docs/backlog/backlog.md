...
| STORY-049 | A cold start is empty, and the app says so instead of looking broken | — | M | 3 | **Re-scoped 2026-08-21, at the design session for STORY-048.** Originally "a cold start produces a corpus with text in it", to be met by auto-ingesting a sample PDF. The project owner's decision reversed it: a first run ingests nothing, and the UI recognises that documents must be ingested rather than rendering blank screens that read as failure. `AUTO_INGEST` defaults to false; the machinery stays for anyone who wants it. Every screen distinguishes *no corpus* from *nothing to do* — Review's "Nothing is waiting for review" is true and misleading when there are no documents at all — and names the action without pretending to be it ([STORY-043](#ready) is the action, sprint 5). See [ADR-019](../specs/adr/ADR-019-the-first-run-is-empty.md) |
| STORY-050 | The codebase contains no code the application cannot reach | — | S | 3 | **Closed 2026-08-21 as no change required, on inspection.** The AST sweep's finding was a fact and not a defect. `text_of` is not superseded by `pages_of` as first claimed — `test_pdf_stages.py` calls it 15 times to feed the whole-text stage functions, so it is a used helper that happens not to sit on the ingest path. `attach_authority`, `merge_authority` and `merge_entity` were staged deliberately: [ADR-011](../specs/adr/ADR-011-instruments-have-versions.md) introduces them, states they are not exercised by ingest, and names the future task that will call `attach_authority` — deleting them would contradict a frozen decision, and `architecture.md` already says plainly that nothing writes them. The extraction cache stays for STORY-048 in sprint 4. Nothing was deleted and nothing should be |
| STORY-053 | Planning documents describe the running app, not its library | — | S | 3 | [roadmap.md](../planning/roadmap.md) says DI-2 phases 3–6 have landed and that triage answers the increment's deliverable question. That is true of the code and false of the product: nothing in the app can produce the obligations triage needs (STORY-048). The same sweep should re-check every 'has landed' claim in the roadmap and every capability claim in architecture.md against what a user can actually reach from a cold start |
| STORY-039 | The graph view fits the window it is drawn in | — | S | 3 | **Defect, found by the 2026-08-21 UI audit.** The detail panel is off-screen at every viewport size tested (1280, 1400, 1920, 2560 wide — panel `x` is always `window.innerWidth + 25`), so selecting a node appears to do nothing: the selection works and fires `GET /graph?expand=`, but the result cannot be seen without scrolling right. Cause: `ForceGraph2D` in `GraphExplorer.tsx` is given no `width`/`height`, and the library then sizes its canvas to `window.innerWidth`/`innerHeight` rather than to its flex parent, pushing the 320px `<aside>` outside the viewport. The same view also scrolls vertically by about 90px, because `height: 100vh` now sits below the nav bar added in DI-2 phase 6. Both need the container measured and passed in. Pre-dates DI-2; the nav made the vertical half visible. Likely means the panel has never been usable |
| STORY-040 | Triage only offers comparisons it can actually carry out | — | S | 3 | **Defect, found by the 2026-08-21 UI audit.** Two dead ends on the same screen. The document picker lists every `:Document` — 440 of them on the sample corpus — but only those with a `:DocumentVersion` can be triaged, so 439 choices lead to an empty edition list and no explanation. And the edition picker offers the oldest edition, which always fails: `GET /triage` correctly returns 400 ("supersedes no earlier edition") per [ADR-015](../specs/adr/ADR-015-changes-are-detected-and-ranked.md), and the alert renders, but the UI should not offer a choice it can predict will fail. Filter the document list to those having versions, and either disable the oldest edition or let it be compared against an explicitly chosen `from_version_id` — the backend already accepts one |
| STORY-041 | The app has a favicon | — | S | 3 | **Defect, found by the 2026-08-21 UI audit.** `GET /favicon.ico` 404s on every page load — the only console error the audit found. Cosmetic, and a one-file fix |
