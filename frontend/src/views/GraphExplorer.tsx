...
import NodeObligationsPanel from './NodeObligationsPanel'
        ...
        {focus && !error && graph && activeSelection && (
          <div data-testid="node-detail">
            ...
            {/* R8, F2. Structure down to specific obligations without leaving
                the map. Keyed on the slug so that changing the selection
                remounts it: the panel then cannot render one document's
                answer under another's name, structurally rather than by a
                guard it has to remember to apply. */}
            <NodeObligationsPanel
              key={activeSelection.id}
              slug={activeSelection.id}
